...
import 'scf_form_state.dart';

/// Cubit form level-transaksi Service Call Freezer (PIC + teknisi). Pola
/// PosfFormCubit — persist ke [ScfInfoModel] di [kServiceCallFreezerInfoBox].
class ScfFormCubit extends Cubit<ScfFormState> {
  final String transNo;
  final Box<ScfInfoModel> _infoBox;
  ...
  ScfFormCubit({required this.transNo})
      : _infoBox = Hive.box<ScfInfoModel>(kServiceCallFreezerInfoBox),
        super(const ScfFormState()) {
    _initAsync();
  }
  ...
  Future<void> _saveStateToHive() async {
    final key = _getHiveKey(transNo);
    final info = _infoBox.get(key) ?? ScfInfoModel(transNo: transNo);
