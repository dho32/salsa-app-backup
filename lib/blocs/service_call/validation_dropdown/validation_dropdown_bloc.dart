...
class ValidationDropdownBloc
  ...
  // --- HANDLER BARU: ADD REMARK PHOTO ---
  Future<void> _onAddRemarkPhoto(
      AddRemarkPhoto event, Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;

      // Fokus ke 'After' (sesuai request Akang), tapi logic map disiapkan
      final currentMap = event.isBefore
          ? Map<NoteType, List<CapturedImageDetail>>.from(
              currentState.remarkPhotosBefore)
          : Map<NoteType, List<CapturedImageDetail>>.from(
              currentState.remarkPhotosAfter);

      final currentList =
          List<CapturedImageDetail>.from(currentMap[event.noteType] ?? []);

      // Cek Limit Max 2 Foto
      if (currentList.length >= 2) return;

      currentList.add(event.photo);
      currentMap[event.noteType] = currentList;

      final newState = event.isBefore
          ? currentState.copyWith(remarkPhotosBefore: currentMap)
          : currentState.copyWith(remarkPhotosAfter: currentMap);
  ...
  // --- HANDLER BARU: REMOVE REMARK PHOTO ---
  Future<void> _onRemoveRemarkPhoto(
      RemoveRemarkPhoto event, Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;
      ...
      currentList.removeWhere((photo) => photo.imagePath == event.imagePath);
      currentMap[event.noteType] = currentList;
  ...
  Future<void> _saveToHive(ValidationDropdownLoaded state) async {
    try {
      ...
      final validationEntry = ServiceCallValidationEntryModel(
        ...
        // SIMPAN FOTO REMARK KE MODEL BARU (ID 22, 23, 24)
      ...
      final Map<dynamic, dynamic> currentAssignments =
          assignmentBox.get(state.transNo) ?? {};
      if (state.selectedOutdoorSerialNo != null &&
          state.selectedOutdoorSerialNo!.isNotEmpty) {
    ...
    try {
      ...
      if (initialData != null) {
        if (initialData.remarkPhotosIndoorAfter != null) {
          loadedRemarkPhotosAfter[NoteType.indoor] =
              initialData.remarkPhotosIndoorAfter!;
        }
        if (initialData.remarkPhotosOutdoorAfter != null) {
          loadedRemarkPhotosAfter[NoteType.outdoor] =
              initialData.remarkPhotosOutdoorAfter!;
        }
    ...
    try {
      ...
      final validationEntry = ServiceCallValidationEntryModel(
        ...
        selectedOutdoorNoteBefore:
            stateSaatEventMulai.selectedOutdoorNoteBefore,
        ...
        selectedOutdoorPSINoteBefore:
            stateSaatEventMulai.selectedOutdoorPSINoteBefore,
        selectedOutdoorPSINoteAfter:
            stateSaatEventMulai.selectedOutdoorPSINoteAfter,
        noteRemarkIndoor: remarkIndoor,
        noteRemarkOutdoor: remarkOutdoor,
        noteRemarkPSI: remarkPSI,
        correctSerialNo: existingCorrectSerial,
  ...
  // --- HANDLER LAMA (PERSISTED) ---

  void _onCorrectUnitSerial(
    ...
    try {
      ...
      if (existingKey != null) {
        ...
        if (!isRevert) entry.noteRemark = newNoteRemark;
        await entry.save();
      } else {
      ...
      if (event.isBefore) {
        switch (event.noteType) {
          case NoteType.indoor:
            newState =
                currentState.copyWith(selectedIndoorNoteBefore: event.note);
            break;
          case NoteType.outdoor:
            newState =
                currentState.copyWith(selectedOutdoorNoteBefore: event.note);
            break;
          case NoteType.outdoorPsi:
            newState =
                currentState.copyWith(selectedOutdoorPSINoteBefore: event.note);
            break;
        }
      } else {
        switch (event.noteType) {
          case NoteType.indoor:
            newState =
                currentState.copyWith(selectedIndoorNoteAfter: event.note);
            break;
          case NoteType.outdoor:
            newState =
                currentState.copyWith(selectedOutdoorNoteAfter: event.note);
            break;
          case NoteType.outdoorPsi:
            newState =
  ...
  void _onSelectOutdoorSerial(
    ...
    if (state is ValidationDropdownLoaded) {
      final newState = (state as ValidationDropdownLoaded)
          .copyWith(selectedOutdoorSerialNo: event.serialNo);
      emit(newState);
      _saveToHive(newState);
    }
  }
  ...
  void _onSelectUnitType(
    ...
    final newState = currentState
        .copyWith(selectedUnitType: event.unitType, selectedProblemCards: []);
    emit(newState);
    _saveToHive(newState);
  }
  ...
  void _onAddProblemCard(
    ...
    final currentState = state as ValidationDropdownLoaded;
    if (currentState.selectedProblemCards
        .any((c) => c.selectedProblemId == event.problemId)) return;
    final updated =
  ...
  Future<void> _onAddCapturedPhotoBefore(AddCapturedPhotoBefore event,
      Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;
      final updated =
  ...
  Future<void> _onRemoveCapturedPhotoBefore(RemoveCapturedPhotoBefore event,
      Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;
  ...
  Future<void> _onUpdateMeasurementBefore(UpdateMeasurementBefore event,
      Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;
      final updated =
      ...
      final newState =
          currentState.copyWith(capturedMeasurementsBefore: updated);
      emit(newState);
      await _saveToHive(newState);
    }
  }

  Future<void> _onAddCapturedPhotoAfter(AddCapturedPhotoAfter event,
      Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;
      final updated =
  ...
  Future<void> _onRemoveCapturedPhotoAfter(RemoveCapturedPhotoAfter event,
      Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;
  ...
  Future<void> _onUpdateMeasurementAfter(UpdateMeasurementAfter event,
      Emitter<ValidationDropdownState> emit) async {
    if (state is ValidationDropdownLoaded) {
      final currentState = state as ValidationDropdownLoaded;
      final updated =
      ...
      if (index != -1)
        ...
        updated.add(event.measurement);
      final newState =
          currentState.copyWith(capturedMeasurementsAfter: updated);
      emit(newState);
      await _saveToHive(newState);
    }
  }
